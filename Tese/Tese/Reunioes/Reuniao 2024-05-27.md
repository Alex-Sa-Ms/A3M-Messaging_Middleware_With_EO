...
The socket middleware-level protocol is identified by **`0x01`**.
### Extended Header
The extender header of this protocol includes:
- **Source Identifier (4 bytes)**: Identifier for the source entity (socket or node).
- **Destination Identifier (4 bytes)**: Identifier for the destination entity (socket or node).
### Message Types
...
### Formato control-socket
	- tag do socket fonte - para indicar
	- tag do socket destino
