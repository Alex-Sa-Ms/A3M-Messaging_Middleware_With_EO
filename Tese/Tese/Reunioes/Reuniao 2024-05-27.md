...
### Terminating a link 
<span style="color:orange"><p>TODO - Write simplification of the cancelling process
</p>
Think about:
<p>- UNLINK arrives before LINK
<p>- UNLINK arrives before LINKACK
<p>- 2nd LINK arrives before UNLINK for the 1st LINK 
<p>- Cancelling when the network is partitioned
</span>
As a link can be established, it can also be severed. For simplification, we will the call the process of severing a link as *unlink*. In addition to severing an established link, the unlink process can also be used to cancel an ongoing link establishment.

This procedure will consist, essentially, in setting the state of the link to "unlinking"[^1], sending an `UNLINK` message to trigger the deletion of all information related to the link (if existent) and waiting for an `UNLINK` message to confirm such deletion. Upon receiving the `UNLINK` message, the socket also deletes all information related to the link.

[^1] Marks that any message received other than an `UNLINK` message should be discarded.
#### Cancelling an ongoing link establishment process
To cancel an ongoing link establishment it is important to consider that the underlying transport protocol (Exon) does not provide ordering guarantee, which means that an `UNLINK` message may arrive before a `LINK` or `LINKACK` message even though it was sent after them.

The only special situation that needs to be considered is the situation where an `UNLINK` message arrives before a `LINK` message. A `LINK` messages marks the beginning of a link establishment process. Prior to receiving this message, the socket does not hold any information about the socket that wants to establish the link, therefore it does not have a link to severe. However, if we assume that there aren't byzantine sockets[^1], this message can be used to anticipate that a `LINK` message will eventually arrive and that it should be rejected. After rejecting the `LINK` message and discarding the state related to the requester, an `UNLINK` message must be sent to confirm that the link was not established.

[^1] This prototype is being conceived to be used in safe environments. To be used in a hostile environment a lot of security features would be required to safeguard against malicious behavior, such as encryption, authentication, authorization, blacklists, etc.

The remaining possible situations follow the same pattern. When the `UNLINK` message is received all state associated with the link is discarded, then an `UNLINK` message is sent to confirm that the link was severed. Any `LINKACK` message received that does not have a corresponding state is discarded.
#### *UNLINK* message arrives before *LINK* message

A `LINK` messages marks the beginning of a link establishment process. Prior to receiving this message, the socket does not hold any information about the socket that wants to establish the link, therefore it does not have a link to severe. However, if we assume that there aren't byzantine sockets[^1], this message can be used to anticipate that a `LINK` message will eventually arrive and that it should be rejected. Upon receiving the `LINK` message all the data related to the requester may be discarded.

[^1] This prototype is being conceived to be used in safe environments. To be used in a hostile environment a lot of security features would be required to safeguard against malicious behavior, such as encryption, authentication, authorization, blacklists, etc.
#### *UNLINK* message arrives before *LINKACK* message

This situation is described by an `UNLINK` message arriving before a `LINKACK` message. This situation can be further divided into two situations: (1) `LINKACK` message has been sent; (2) `LINKACK` message has not been sent. In situation 1, if `LINKACK` message has been sent,
Having seen the link establishment process, we know that a socket waiting for a `LINKACK` message is waiting for the decision of the other socket.



Before diving into each of the scenarios, sending an `UNLINK` message should be sent only if the socket's decision has already been shared. If the socket's decision has yet to be shared, a `LINKACK` message can be sent immediately, containing a negative decision which indicates that the link establishment was cancelled. This `LINKACK` message may be sent immediately.

**`UNLINK` message arrives before `LINK` message:**
A receiver engages in a link establishment process when it receives a `LINK` message. An `UNLINK` message that arrives before the receiver engages in a link establishment process is pointless has the socket cannot cancel what has not yet begun. To face this problem, I thought of three solutions:
- **Send an ERROR message** informing that the link establishment process has not yet begun. Receiving the `ERROR` message should result in the re-transmission of the `UNLINK` message.
	- Increases used bandwidth, as messages need to be re-transmitted;
	- May result in lower latency. By not delaying the transmission of the `UNLINK` message and assuming that it will most likely arrive after the `LINK` message, the cancel operation will have the lowest latency possible. However, the `UNLINK` message may arrive first as referred previously, which results in the re-transmission of messages until the `UNLINK` message arrives after the `LINK` message and thus increasing the latency.
- **Ensure order in the delivery of link-related messages**
	- Unnecessary complexity? Requires creating a queue and identifiers exclusive to link-related messages.
- **Include UNLINK-related states in the state machine**.
	- (1) Receiving `UNLINK` before `LINKACK` or (2) receiving `LINKACK` with a "CANCEL" decision before `LINK`, should result in the transition to a state that postpones cleaning procedures until the `LINKACK` message arrives in (1) and the `LINK` message arrives in (2).
		- The cleaning procedure is removing all data related to the now unlinked socket. 
	- Essentially, the goal of this solution is creating a convergent state machine which ends up in the same state independently of the order of the messages received.
	- *Since cancelling the establishment of a link is possible, cancelling the operation of cancelling the link establishment, and so forth could also be possible, however that would result in unnecessary complexity.* To accomplish such feature, identifying the linking process is required. This identifier must be convey information about the order, that way, a message received that is related to an earlier linking process can be ignored. The problem is that any socket may issue the establishment of link. That means, both sockets can issue the linking process, and therefore, the linking process must distinguish which process should win for the state to converge. The criteria to select the process that should win can be alphabetic order of the identifier of the node followed by socket's tag. The combination of node identifier with a socket tag makes a globally unique identifier, therefore, making it possible to converge the state.  

<b style="color:red">
<p>- Cancelling section should appear after unlinking section.</p>
<p>- Unlinking process should mentioned that a destination marked to be unlinked may no longer be used to deliver messages. However, messages may still be received until the other socket confirms the destruction of the link.</p>
...
<p>- Having a three-way handshake can be important as it allows the flow of messages to terminate between the two sockets before effectively closing the link. Assume that the socket that initiates the unlinking process is called A and the other socket is called B. The first message, sent by A, is used to advertise the intention of closing the link. The second message indicates that B is ready to close the link and won't send anymore messages. The third message, sent by A, confirms that A won't send anymore messages and that B may close the link. Requesting receipts for the messages may help decide when the messages should be sent to avoid implementing ordering mechanisms.<p>
(receipts may be helpful for a lot of situations)
