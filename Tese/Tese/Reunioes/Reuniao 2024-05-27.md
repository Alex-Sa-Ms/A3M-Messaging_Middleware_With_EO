...
Issuing a linking process and then cancelling it is a possible scenario and thus should be handled appropriately. This procedure is part of the unlinking process, and happens when *unlink()* is called before the link has been established. 
#### 
Exon provides exactly-once delivery guarantee, however, it does not provide any ordering guarantee. This means that all messages will eventually be delivered to the target application but may arrive in an order that differs from the sending order. Having this in mind is essential to successfully cancel the establishment of links. 

The following scenarios should be considered:
1. `UNLINK` message arrives before `LINK` message;
2. `UNLINK` message arrives before `LINKACK` message;
3. `UNLINK` message arrives before `LINKACK` message with metadata.
...
<b style="color:red">
<p>- Cancelling should appear after unlinking is explained.</p>
...
<p>- Having a three-way handshake can be important as it allows the flow of messages to terminate between the two sockets before effectively closing the link. Assume that the socket that initiates the unlinking process is called A and the other socket is called B. The first message, sent by A, is used to advertise the intention of closing the link. The second message indicates that B is ready to close the link and won't send anymore messages. The third message, sent by A, confirms that A won't send anymore messages and that B may close the socket. Requesting receipts for the messages may help decide when the messages should be sent to avoid implementing ordering mechanisms.<p>
