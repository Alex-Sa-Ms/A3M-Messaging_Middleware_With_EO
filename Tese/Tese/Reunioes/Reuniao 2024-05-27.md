...
For a link to be established, both sockets must agree in linking with the counterpart socket. A socket establishes the link on its end when it has formulated its own decision, has received the counterpart socket decision and both decisions indicate acceptance. For a socket to formulate a decision, it requires the metadata of the counterpart socket. Therefore, to gather both decisions, both sockets must share their metadata, formulate and share their decision.

To accomplish the above, two types of messages are employed: `LINK` and `LINKACK`. The `LINK` message initiates the linking process and carries the socket's metadata. The `LINKACK` is used to send the socket's decision, however, it can also include the socket's metadata if it has not been sent yet.

Having seen the simplified explanation of the process, the main possible scenarios are the following:
...
- **Scenario 2 (Both sockets initiate the link establishment process):**
...
- The remaining possible scenarios can be verified through the examination of the state machine diagram presented further ahead. 


// refer this in the detailed explanation // , avoiding the need to send two separate messages: a `LINK` message and a `LINKACK` message. 

There are many scenarios that can happen, all of which are contemplated in the state machine diagram below. However, before diving into the complexity, let us explore the two main scenarios.
