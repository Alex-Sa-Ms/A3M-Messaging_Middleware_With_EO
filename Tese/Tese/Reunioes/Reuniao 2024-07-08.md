...
Waiting for a link to become available for sending a message is crucial for developing blocking send methods for various socket types. 

While some socket implementations can be easily implemented using a generic send method which includes the waiting functionality, other implementations may require a method that exclusively waits for the sending availability, enabling the socket to decide if and what message should actually be sent.

Having said that, in addition to the generic blocking send method, two key functionalities are needed: waiting for a specific link to become available (`waitForLinkAvailability(sid: SocketIdentifier)`) and waiting for any link to become available (`waitForAvailableLink()`).
...
1. **Fairness/Starvation**: The main problem is to avoid prioritizing link-specific[^1] or general threads[^2].  Evenly distributing link credits between waiting threads and preventing threads from never being signaled are the problems that follow depending on the fairness of the signaling mechanism chosen by the client.

2. **Thundering Herd**: This problem is addressed by avoiding signaling threads that cannot perform work, thereby preventing unnecessary context switches that degrade performance. Signaling a thread when the return condition cannot be met results in two unnecessary context switches since the thread needs to await again.
...
The key for facing the mentioned challenges is to use requests. There are two types of requests: link-specific and general. With each request corresponding to the will of a thread to wait for sending availability, the thundering herd problem is fixed by only signaling the amount of threads that can be satisfied by the credit balance that each link allows. Regarding the starvation and fairness problems, two solutions are provided for the client to choose from: a *fair-ish* version and an unfair version.   

### Fair-ish implementation

To address the fairness and starvation problems, the devised solution involves requests that offer a sense of order. This order is achieved by giving to each request what we can call a ticket number. Each request is then placed in an appropriate queue depending on its type. Each link has its own queue of link-specific requests, and general requests are kept in a common queue. When a link's credits are replenished, it signals waiting threads based on the ticket numbers of the requests. The link peeks at the first ticket number of both its own link-specific requests queue and the general requests queue. The queue with the smallest ticket number (or with requests) dictates which type of waiting thread should be signaled. This approach solves the fairness/starvation problem between different types of threads. To ensure fairness among threads of the same type, a fair version of the lock can be employed.

To prevent new "waiting" threads from stealing credits from threads already waiting, requests have three states: new, unsettled, and settled. A request is new when created, becomes unsettled when a thread is signaled, and becomes settled (deleted) when the thread reacquires the lock and secures the credit. A new "waiting" thread can return immediately only if the link credits exceed the combined number of new and unsettled requests. By allowing this immediate return, the implementation is not completely fair, but for efficiency purposes this solution seemed acceptable as it prevents two unnecessary context switches: the first when threads enter the waiting queue; and the second when they are signaled shortly after.

<span style="color:red">Usar algoritmo de sinalizacao em funcao do nº de threads a sinalizar?</span>
### Unfair Implementation

The unfair implementation is much more relaxed on the fairness and starvation problems. Apart from interleaving (when possible[^1]) signals of link-specific and general threads, there is no attempt in signaling threads in the order in which the requests were issued. Furthermore, credit stealing is not prevented. A thread that in the fair-ish version would be queued, due to the insufficient amount of credits to satisfy both the thread in question and the threads already waiting, can consume the credit instantaneously.

[^1] Interleaving is only possible when there are requests of both types.
...
In addition to the blocking send method, having "waiting" methods provides flexibility for implementing different types of sockets. These "waiting" methods, as their name dictates, should only wait, therefore, it does not make sense for them to consume a link credit if a message is not being sent. Because both the non-consumption of credits and avoidance of unnecessary signaling of threads are desirable, new problems arise. 

The first problem is starvation. If a signaled thread, which returned from the waiting method successfully, does not consume a credit, another thread that could have been signaled will starve until a method that increases the credit balance is used. Unlike general threads, a link-specific thread requires the increase of credits of the link of interest. To avoid such situation, a method called `releaseCredit()` must be invoked when the thread decides to not send a message after waiting for the sending availability.  By "releasing" the credit,  another waiting thread can be signaled.

However, the `releaseCredit()` does nothing more than signaling a new thread. Waiting for a credit does not reserve it. An increase of credits, as explained above, would undo the reservation. This means that reserving multiple credits in one or multiple links is not possible. Although it is not clear in which situation it may be used, providing such possibility does not hurt.

So, the next solution that came to mind was effectively consuming the credit

However, the `releaseCredit()` method also poses a problem, we need a way to prevent this method from inappropriate use, which could lead to a state of inconsistency between the credits at the sender and the credits that the receiver expects the sender to have. 
(Nao esta correto factualmente, já que afirmei que o método wait nao deveria consumir crédito)

- Falar: Permitir reservar créditos possibilita esperar pela disponibilidade de múltiplos links, antes de enviar as mensagens, por exemplo, se for pretendida sincronizacao. No entanto, num contexto exactly-once isto nao parece muito certo.
