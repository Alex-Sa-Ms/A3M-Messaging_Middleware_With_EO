...
## Solution 2
- Create a queue for all these types of events.
- Share the queue used to wait for any link?
	- Default socket implementation could have a method that allows adding a new waiter (with a custom function) and sending a custom event.
- This should be acceptable since these events are not the main functionality.
- If the functionality is assumed to be used a lot, then a custom queue for that type of event could be created and exposed by the custom socket implementation.
## Questions
### How to implement with poller mechanism?

<b style="color:red">----- TODO -----</b>

# Flag "notifyIfNone" 
- A flag `notifyIfNone` quando definida num método, tem como objetivo a emissão de uma exceção aquando da ausência de links estabelecidos e por estabelecer. 
- Isto permite que peças (dinâmicas) da topologia, que são responsáveis por iniciar processos de estabelecimento de links, não fiquem eternamente bloqueadas. O seu eterno bloqueio resulta de ser o iniciador de links, logo não recebe pedidos para estabelecer links, e possivelmente porque os pedidos de estabelecimento efetuados foram recusados. Se o método não retornar, para este tipo de peças, quando não existem ligações estabelecidas ou no processo de serem estabelecidas, então o programa ficará num estado de bloqueio.
- Para peças fixas da topologia, que não iniciam ligações, esta funcionalidade não é um requisito, já que estas necessitam aguardar pelos pedidos de ligação oriundos de outras peças. 
- O comportamento deve ser solicitado através de uma flag já que para uma peça fixa da topologia, o aviso da inexistência de links não é desejável (seja por uma exceção ou de outra forma), já que obrigaria a uma espera ativa.
## Solution 1
- Como os sockets são thread-safe, a situação de falta de links, poderia ser resolvida por uma outra thread, no entanto, por conveniência e para facilitar a programação, pensou-se no uso desta flag.
	- A solução implícita a realizar por outra thread seria descobrir novas ligações.
- A flag é primariamente empregada pelos métodos genéricos `waitForLink()` e `waitForLinkAvailability()` que esperam pelo estabelecimento de um link ou pela disponibilidade para envio de um link estabelecido, respetivamente.
 - Esta flag pode ser utilizada também em métodos de envio e receção.
	 - O `AbstractSocket` pode ter uma implementação base dos métodos de envio e receção com o argumento `notifyIfNone`. Estas implementações devem emitir por base a exceção `UnsupportedOperationException` para demonstrar que não foram implementadas. Se for decidido que se pretende implementar esse método, então faz-se o @Override e expõe-se esses métodos na classe *wrapper* do Socket.
## Comments after epoll() adaptation
- These methods were required before, so they are required now as well.
- Any method that suggests "**waiting**" for something **must not be an exclusive poll operation**.
- Methods that execute an operation can be exclusive, such as receiving and sending messages.
- Links that are being established must have their poll() method return an empty event mask and allow queuing of waiters.
### `waitForLink(sid : SocketIdentifier)`
- Its purpose is to wait for the *establishment of **a** link*.
- Must:
	- Use `Poller.poll()` for a *non-exclusive* and occasional poll operation.
	- Inform link closure, when `POLLHUP` flag is received, through the return value (throwing an exception is not required as checking the state is the only purpose of the method);
	- If any other flag is received - `key & ~POLLHUP != 0` - then the call can return successfully;
	- Otherwise, if a timeout was provided and expired, return a value that informs such case.
### `waitForLink() : SocketIdentifier`
- Its purpose is to wait for the *establishment of **any** link*.
- **Ideas:**
	- **Idea 5** ***(BEST FOR NOW)*** : Wait queue to report established links and lack of any link.
		- Separate collections for links being established ("establishing") and established("ready"/"closing").
			- When a link becomes "closed", meaning no operation can be realized, it must be removed immediately.
		- Algorithm:
			1. Iterate over the "established" links collection and return the first one that is not closed.
			2. If there isn't any link that meets those requirements, check if there are "establishing" links. 
				1. If there aren't links being established and `notifyIfNone` is set, throws exception to inform that there aren't any links. 
				2. Else, register the thread, as non-exclusive, in a special queue reserved for this purpose. The *wait queue function* should not delete the entry, it should use the default wake-up using the `ParkState` when the *key* contains an identifier or when the *key* is `null` and the `notifyIfNone` was set by the thread.
					- The identifier event is triggered when the link becomes established.
					- The `null` event is triggered when a link establishment was canceled and that link was the only one in both collections.
					- The key should be used to update an atomic reference shared by all waiters.
						- The key could be set for each waiter in their private object, however, the purpose of this method is just to inform when a link has been established.
						- The key could be updated using a shared atomic reference, however, to avoid all threads from getting the same 
			3. The woken-up thread should check if the reason for the wake-up is still valid or not. If it is valid, return it. If not:
				1. Re-checks if the "established" collection has any link to be returned.
				2. If timeout expired, returns the value obtained from the "established link" or `null` if there isn't one.
				3. If timeout hasn't expired, check if the `notifyIfNone` flag is set and the notification conditions are met.
				4. If it has not returned or thrown an exception yet, park again and wait for a notification.
	- **Idea 1** : Use `Poller.poll()`.
		- **Problems:** 
			- Re-queuing can happen: A `POLLHUP` would result in the return of the polling operation, making it necessary to poll again and queue the thread again in all links.
			- Endless and delayed waiting can happen: New links added after the invocation cannot be added until returning and a new poll is executed with the current collection of links. The new links are only contemplated if the return isn't already successful.
	- **Idea 2** : `Poller` instance for each invocation of the method
		- Fixes re-queuing.
		- **Problems**:
			- Endless and delayed wake-up can happen.
	- **Idea 3** : `Poller` instance shared between all invocations of this method
		- Fixes re-queuing.
		- Fixes endless/delayed wake-up since adding/deleting interest in links is possible.
		- **Problems**:
			- Cannot wake up threads when links do not exist.
	- **Idea 4** : Shared `Poller` instance + "special" pollable to inform lack of links  
		- Fixes re-queuing;
		- Fixes endless/delayed wake-up.
		- Can wake-up when links do not exist.
		- **Problems**:
			- Cannot wake up threads based on the `notifyIfNone` flag.
- Additional ideas:
	- Use reference count to remove mechanism when not being used to avoid unnecessary memory consumption?
### `waitForLinkAvailability()`
- These methods are not required if the links are exposed safely and in a manner that enables the polling mechanism to be used.
### `send() & receive()`
- Ignore fairness for now. If sending/receiving is possible instantaneously, then do it without queuing. Otherwise, queue directly as an exclusive waiter.
# Handling messages
- If data messages are fed to link objects, and then received through them, it is possible to update the flow control window automatically, instead of making the sockets give the credits manually.
- Messages must always be pre-checked by the default socket feed method. This feed method filters and acts on default logic messages, such as flow control.
- Regarding **custom messages**:
	- **Idea 1 *(BEST)*** : 
		- **Design**: 
			- Pass **all custom** messages, both control and data, to the custom feed method. 
			- Data messages are queued depending on the return value returned by the custom feed method. The return value must either imply "queue" or "do not queue".
				- Some action may need to be performed on the socket using the data message, but require the message to be queued still.
			- If the return value implies "do not queue", the data message is assumed as handled by the flow control mechanism, meaning a credit can be provided for the source to send a new data message. 
			- Only data messages are queued in the link's incoming queue. 
				- Separate queue for data messages to enable immediate polling from the queue. 
				- If the queue was shared with control messages, searching for the first data message would be necessary, leading to inefficiency.
			- Control messages must be handled immediately. 
				- A queue for control messages is not created since most socket semantics will not require postponing the processing of a control message. If postponing the processing of the custom control message is required, the socket must queue the message in a data structure of its creation.
			- Not handling a control message means discarding the message.
			- Custom socket may provide a custom queue producer, which enables the link to choose the perfect fit regarding its queuing semantics. The method must receive the link (or enough information about it, like its type) for the appropriate queue to be attributed.
				- Different specific orders may need to be employed depending on the compatible type with which a link is established.
		- **Advantages**:
			- Complete control over messages.
			- Automatic flow control. When a message is removed from the incoming queue, it can be assumed as handled.
			- No searching required to poll a data message.
			- Links can implement `Pollable` to be used with `Poller`.
		- **Disadvantages**:
			- \\<nothing\>
	- **Idea 2** : Pass all **custom-control messages** to custom feed method. **Data messages** could be passed directly to the appropriate link object enabling even the exposure of the link to the user for custom polling.
		- **Advantages**:
			- Prioritization of custom-control messages.
			- Automatic flow control.
			- Data messages are still available for de-queuing at anytime for processing.
			- Automatic read polling event for the link.
		- **Disadvantages**:
			- \\<nothing\>
		- **Enhancements:**
			- Since knowing instantly when **data messages** may be desired, a "notification" could be sent to the custom feed method, allowing the socket to handle it instantly if desired. If the data message was marked as handled (be it using the return value or in the notification object), then the data message would not be queued in the link object. If not marked as handled, the message would be added in the tail of the queue.
				- Superior to waiting using the polling mechanism. Even implementing the priority flag in the wait queues would not be as advantageous as this.
	- **Idea 3 :** Pass all custom messages to the link object. Since messages can have priorities, a `Comparator` should be provided by the socket to store the messages in an ordered manner. If not, the default `Comparator` is give priority to all custom control messages and data messages come after. 
		- **Problems**:
			- If data messages and control messages are put in the same queue, de-queuing of data messages needs to iterate until the first data message is found. 

## Sub-problems
### Should custom socket have free access to the link queues?**
- **Advantages**:
	- Tailored queuing of data messages.
- **Disadvantages**:
	- If not correctly employed, can hinder the automatic behavior of the flow control mechanism.
- **Other Solutions**:
	- **Idea 1** : Custom `Comparator`
		- Should be enough.
	- **Idea 2** : Custom queuing function
		- Seems a bit excessive.
	- **Idea 3** ***(Best - enables both previous ideas)*** : Custom queue producer.
		- Each socket may provide a custom queue, employing a `Comparator` or custom queuing semantics. For instance, if order is required, one can create a custom queue class that extends the class of interest but only allows polling when the intended message is at the head of the queue.
### Data & control messages cannot be put in the same queue
#### Rationale
Hinder de-queuing each type of message. For instance, if a waiter wants to dequeue a data message it needs to iterate over the queue until the first data message is found.
### Data messages may require custom handling
#### Rationale
Custom sockets may want to include custom information in data messages, which should be extracted before passing the message to the user. How can this be done if the message is queued in the link's queue? 

I was assuming that a data message would only contain user data, however, since only one type of messages will be controlled by flow control, i.e. the data messages, the custom socket may want to have multiple types of data messages. For that to happen, the custom socket would need to add custom data in the message payload, making parsing the payload and extracting the actual user message from the payload, to than give it to the user. 
#### Solution
##### When link related operations are not exposed
When the socket does not allow operations to be performed directly over a specific link, like receiving from a specific link, or using a polling mechanism over a combination of links, the socket can easily solve the problem by polling the message from the link's queue, handling it as needed and returning the transformed data message.
##### When link operations are exposed
When exposing the link functionality is required, the custom socket must create its own API that serves as a bridge to the link functionality, but not expose the links directly without protection. For instance, if sending/receiving a message for a specific link is wanted, such methods should receive the identifier of the link as a parameter and perform the operation without exposing the link. When polling links is of interest for the users, the socket may provide methods that receive the `Poller` instance and the link identifier as parameters, and proceeds to find the link instance and add it to the `Poller` instance. This functionality does not pose a problem, as the link objects are not exposed by the `Poller` instance, only the identifier is used to associate the event mask with the associated `Pollable` object.  

# Wait for link availability(to send)**\[Solution outdated by epoll()\]**
...
- **"notifyIfNone" alternative** (to describe in thesis as well). 
	- **Idea 1:** Use wait queue functions to wake up when such situations happen. When the last pollable is deleted, a global wake up should be performed. 
	- **Idea 2:** Threads that want to be signaled when the poller is empty (i.e. doesn't have pollables registered) can register themselves in a set. When a pollable is removed from the poller (the method may return the number of monitored pollables) and if that number is 0, interrupting the threads registered in the set can be done to wake them up and consequently the interrupted flag check can throw an interrupted exception.
- **nesting** pode vir a ser necessário para permitir criar uma instância com todos os links do socket subscritos, ou, a simples existência de um poller com todos os links registados pode ser suficiente.
- pollables may not be discarded "completely" (for example, releasing the id for use by another pollable) until the wait queue is completely empty. 
	- *Let's not bother with this for now, however, for correctness a ref count like the kernel uses would be ideal to know when the id is "safe" to be reused.*
- 
### Problems
...
# How to write thesis
- Create section initially to inform how to read the document.
- When talking about the process of designing the middleware, do the following:
	- Explain **problem**, **solution** and then another section can be used to talk about the thought-process and walkthrough to get to the solution. This last section should include solutions that were taken into consideration but where not implemented. Another section with future considerations, for example, include more elaborated solutions that were though about but not implemented due to lack of time or because we didn't see it being a required feature for the moment.
	- The writing should be set up in this way so that the reader may walk through the problems without being lost on all the thought solutions. It can just speed run the document by reading the problem and then the solution. If curious he may read how the solution was obtained.