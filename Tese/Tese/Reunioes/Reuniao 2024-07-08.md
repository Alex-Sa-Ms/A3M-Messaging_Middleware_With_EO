...
- *Cons:*
	...
	- ***Considering that wake up entries may be removed, the logic becomes even more complex. The queue would need a counter of exclusive entries to know when to stop instead of checking if the first notified entry was reached. The size of the queue could also be used along with a local function counter of non-exclusive entries to know how many exclusive entries are there.***
