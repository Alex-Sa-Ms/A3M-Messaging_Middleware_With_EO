...
- Regarding **custom messages**:
	- **Idea 1 *(BEST)*** : 
		- **Design**: 
			...
			- Custom socket may provide a `Comparator` to order the data messages based on a custom criteria. The method must receive the link (or enough information about it, like its type) for the appropriate `Comparator` to be attributed. 
		- **Advantages**:
