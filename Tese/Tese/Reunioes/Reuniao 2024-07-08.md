...
- Regarding **custom messages**:
	- **Idea 1** : Pass all custom messages, both control and data, to the custom feed method. 
		- **Advantages**:
			- Complete control over messages.
		- **Disadvantages**:
			- No automatic flow control.
			- Data messages must be queued when not wanted for processing.
	- **Idea 2** **(Seems the best idea)**: Pass all **custom-control messages** to custom feed method. **Data messages** could be passed directly to the appropriate link object enabling even the exposure of the link to the user for custom polling.
		- **Advantages**:
...
- **Problem: Should the custom socket have free access to the link queues?**
	- If the custom socket implementations are given free access to the link queues, the proper functioning of the flow control mechanism can be affected.
	- However, if not given the access, the messages cannot be reordered as the custom implementation may require. 
	- Could providing a custom `Comparator` be enough? Or a custom queuing function.
